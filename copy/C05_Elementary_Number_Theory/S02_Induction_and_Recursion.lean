...
theorem mul_add (m n k : MyNat) : mul m (add n k) = add (mul m n) (mul m k) := by
  induction' n with n ih
  . simp [zero_add, mul]
  rw [succ_add, mul, mul, ih]
  rw [add_assoc, add_assoc, add_comm _ m]
...
theorem succ_mul (m n : MyNat) : mul (succ m) n = add (mul m n) n := by
  induction' n with n ih
  . rw [mul, mul, add]
  rw [mul, ih, mul, add_assoc, add_assoc, add_comm n, succ_add]
  nth_rw 2 [add_comm m]
  rw[succ_add, add_comm n]
