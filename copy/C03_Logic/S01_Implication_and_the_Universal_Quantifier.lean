...
lemma a: ∀ {a b c: ℝ}, a < b -> b ≤ c -> a < c := by
  exact fun {a b c} a_2 a_3 => (lt_of_lt_of_le a_2 a_3)
...
theorem my_lemma4 :
  ...
  calc
    ...
    _ ≤ |x| * ε := by apply (mul_le_mul (le_refl |x|) (le_of_lt ylt) (abs_nonneg y) (abs_nonneg x))
    _ < 1 * ε := by apply ((mul_lt_mul_right epos).2 (lt_of_lt_of_le xlt ele1) )
    _ = ε := one_mul ε
...
theorem Subset.trans : r ⊆ s → s ⊆ t → r ⊆ t :=
  fun hrs hst => fun x hxr => hst (hrs hxr)
